...
class ChronometerManager:
        ...
        if channel == 1:
            ...
            if self.start_time is not None:
                ...
                if channel == 4:
                    ...
                    if not is_blocked and len(self.left_lane_crossings) < 16:
                        self.left_lane_crossings.append(crossing_time)
                else:
                    # KANAŁ 3 = TOR 2 = LINIA 2
                    self.osf_all_right_crossings.append((crossing_time, is_blocked))

                    if not is_blocked and len(self.right_lane_crossings) < 16:
                        self.right_lane_crossings.append(crossing_time)
    ...
    def update_wachadlo_display(self):
        ...
        if len(self.left_lane_crossings) == 16:
            left_time = self.left_lane_crossings[-1]
            display += f"✅ TOR 1:  {self.format_time(left_time)} s ({len(self.left_lane_crossings)}/16)\n"
        else:
            display += f"⏳ TOR 1:  {len(self.left_lane_crossings)}/16 przecięć\n"

        if len(self.right_lane_crossings) == 16:
            right_time = self.right_lane_crossings[-1]
            display += f"✅ TOR 2: {self.format_time(right_time)} s ({len(self.right_lane_crossings)}/16)"
        else:
            display += f"⏳ TOR 2: {len(self.right_lane_crossings)}/16 przecięć"

        self.update_display(display)

        if len(self.left_lane_crossings) == 16 and self.left_lane_result is None:
            left_time = self.left_lane_crossings[-1]
        ...
        if len(self.right_lane_crossings) == 16 and self.right_lane_result is None:
            right_time = self.right_lane_crossings[-1]
        ...
        # === NAPRAWA: Gdy oba tory skończą - zaktualizuj GUI ale NIE zatrzymuj timer_running ===
        if len(self.left_lane_crossings) == 16 and len(self.right_lane_crossings) == 16:
            if not self.race_completed:
                self.race_completed = True
                self.next_race_btn.config(state='normal')
